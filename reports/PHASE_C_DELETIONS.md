...
### Task 1: 101 shared-template junk files (linguistics `part`)

All 101 files listed in `delete_candidates.txt` for this round (slug, path, `inbound 0` — zero incoming links) were manually reviewed body-by-body. Every one confirmed as generic, copy-pasted English template prose unrelated to its specific title (e.g. a "bilingualism" concept file whose body is the same boilerplate "Bilingualism is competence in two languages..." paragraph shared verbatim across a dozen unrelated slugs; a "phonology" boilerplate shared across `con-forensic-phonetics`, `con-gestural-phonology-browman`, `con-palatalization-sound-shift`, etc.; a "semantics" boilerplate shared across a dozen more; a "sociolinguistics" boilerplate; a "neurolinguistics" boilerplate). None had topic-specific content — not even the thinker files (`thk-broca-localization`, `thk-wernicke-discovery`, `thk-labov-sociolinguistic-method`, `thk-haspelmath-comparative`, `thk-keith-johnson-acoustic-phonetics`, `thk-kent-johnson-formant-tracking`, `thk-mark-johnson-linguist`, `thk-peter-ladefoged-acoustics`) contained any biography — their bodies were the same generic subfield-boilerplate text as the concept files.

Recorded all 101 (slug, Arabic title, `en:` title, grouped by `part` — all were `part: linguistics`) in `reports/TOPICS_TO_WRITE.md` for future proper writing, then deleted all 101 files. Re-checked for inbound references across `content/ar` (excluding `drafts/` and `_merged`) both before and after deletion: zero references found, so no `related`/`edges` cleanup was needed anywhere.

None of the 101 slugs were in `protected.txt` for this round.

### Task 2: leftovers — unverifiable/fabricated people

- `thk-jdavid` ("Jordan David") — re-ran WebSearch for "Jordan David" + social therapy + Fred Newman/Lois Holzman/East Side Institute. No matching person found; confirms the file's own prior `status: quarantined` finding (the only documented figure in that role, Dan Friedman of Castillo Theatre, was never substituted in). Deleted the file. Removed the live `related:` list entries pointing to it in `thinkers/thk-mrothery.md`, `thinkers/thk-dparker.md`, and `thinkers/thk-jtolman.md`. (Separate `gaps`-note prose mentioning the slug historically, in `thk-ramsden.md`, `thk-dparker.md`, and `thk-jtolman.md`, was left untouched as it is a documentation record, not a live link.)
- `thk-leehyungjun` ("Lee Hyung-Jun", 이형준) — re-ran WebSearch for "Lee Hyung-Jun" + hwabyung + ACT + Korean psychiatrist. No matching person found; known researchers in the field are Lee Si-hyung, Min Sung-gil, and Kim Jong-woo — none matches. Confirms the file's own prior `gaps` finding. Deleted the file. Removed the `related` entries pointing to it in `thinkers/thk-kimkwansung.md`, `schools/sch-humanistic.md`, `schools/sch-positive-psychology.md`.
- `thk-brueger` ("Bernd Rüger") — not on the leftovers list in this round's spec by name, but discovered while tracing `thk-jdavid`/`thk-leehyungjun` cross-links; the file's own `gaps` already documented an independent WebSearch (Sept 2026) finding no record of the person in WAPP board lists or Wiesbaden therapist directories, recommending quarantine review. Re-confirmed via WebSearch this round: no match. Deleted the file (was already flagged, not actually quarantined). Removed the `related` entries pointing to it in `thinkers/thk-npeseschkian.md` and `thinkers/thk-fpeseschkian.md`.

### Task 2: duplicate merges

- `concepts/con-shunyata.md` ← `concepts/con-shunyata-emptiness.md`: confirmed same Śūnyatā/Nāgārjuna/Madhyamaka topic (the dup's own `gaps` note already flagged this as an unresolved near-duplicate). Merged the dup's one unique element — the Mūlamadhyamakakārikā source citation — into `con-shunyata`'s (previously empty) sources section. Repointed 8 external references (`wrk-religion-and-nothingness-nishitani`, `con-upaya-skillful-means`, `con-two-truths-doctrine-buddhist`, `con-trisvabhava-three-natures`, `con-one-mind-two-aspects-wonhyo`, `con-kshanikavada-universal-momentariness`, `br-madhyamaka-svatantrika`, `br-madhyamaka-prasangika`) from `con-shunyata-emptiness` to `con-shunyata`. Deleted `con-shunyata-emptiness.md`. (Neither slug was in `protected.txt` this round, unlike a prior round where `con-shunyata-emptiness` was blocked.)
- `concepts/con-being-toward-death.md` ← `concepts/con-being-towards-death.md`: confirmed same Heideggerian *Sein-zum-Tode* concept (both files' own `gaps` notes already flagged this as a near-certain toward/towards spelling duplicate). Merged the dup's unique `related` entries (`con-authenticity`, `con-das-man-the-they`), its `active_start: 1927`, and added a new "الأصالة وزيف الجموع" section summarizing its content into the canonical file. Repointed the 1 external reference (`works/wrk-being-and-time-heidegger.md`) from `con-being-towards-death` to `con-being-toward-death`. Deleted `con-being-towards-death.md`.
- `schools/sch-refugee-displacement-anthropology.md` ← `schools/migration-displacement-school.md`: confirmed overlap — the dup was an empty template stub ("مدرسة أنثروبولوجية معاصرة متخصصة في...") with no real content, no sources, and no external references, entirely subsumed by the canonical, fully-developed file. Nothing to merge. Deleted `migration-displacement-school.md`; logged the merge decision in the canonical file's `gaps`.

### Additional broken-link fixes found by the checker

The first `check_content_integrity.py --lang ar` run after the Task 1 deletions surfaced 10 dangling `related` references that pointed at deleted-in-this-round slugs but had not been caught by the pre-deletion inbound-reference scan (their `- id:` lines used a title text that differed slightly from what grep had matched against, or were added/found only on the second pass). Removed the dangling `related:` list entries in: `thinkers/thk-joseph-greenberg.md` (→ `con-genetic-classification-african-languages-greenberg`), `concepts/con-adjacency-pairs-turn-taking.md` (→ `con-silence-pragmatics`), `concepts/con-conceptual-metaphor-theory.md` (→ `thk-mark-johnson-linguist`), `concepts/con-code-switching-bilingualism.md` (→ `con-intra-sentential-code-switching` and `con-inter-sentential-code-switching`), `schools/sch-linguistic-typology.md` (→ `con-grammatical-gender-noun-classes`), `schools/sch-variationist-sociolinguistics.md` (→ `wrk-sociolinguistic-patterns-labov`), `schools/sch-conceptual-blending-theory.md` (→ `con-optimality-principles-blending`), `schools/sch-aphasiology-communication-disorders.md` (→ `sch-neurolinguistics-brain`).

### Checker

`python3 scripts/check_content_integrity.py --lang ar` run again after the above fixes: **فُحص 12991 ملفاً من 12991. ✅ سليم — لا مشكلات.** (12,991 files checked, 0 problems.)

## Round 5 merges

### Spec pairs

- `instruments/ins-mmse` ← `instruments/ins-mmse-mini-mental-state`: both were the Folstein MMSE. Kept slug `ins-mmse` (canonical `id: INS-0054`, required by spec); the dup had far richer content (full scoring table, cutoffs by age/education, reliability/validity breakdown, adaptations, sources) despite the canonical having the proper frontmatter (`items_count`, `reliability`, `psychometric_standing`, etc). Rewrote `ins-mmse.md`'s body merging both — structure/cutoffs/psychometrics/limitations table from the dup folded into the canonical's narrative, and added the dup's 3 numbered sources. No inbound references pointed at the dup slug, so no repoints needed. Deleted `ins-mmse-mini-mental-state.md`.
- `thinkers/thk-ekandel` (Eric Kandel, kept per spec — 13 inbound references vs. 0 for the other): `thk-kandel-learning-memory` was an empty template stub ("مساهمات وأبحاث Eric Kandel في علم اللغة المعاصر") with `major_works: []`, `edges: []`, `related: []` — nothing to merge, no inbound references. Deleted it directly.
- `schools/sch-variationist-sociolinguistics` ← `schools/sch-sociolinguistics-variation`: canonical was already the fully-developed Labovian-tradition article (32 inbound references); the dup was a thin English-language stub. Added its one non-duplicate source (Eckert 2000, *Linguistic Variation as Social Practice*) to the canonical's sources. Repointed 7 inbound references (`schools/sch-code-switching-advanced.md`, `thinkers/thk-kevin-stratos.md`, `thinkers/thk-cynthia-blanton.md`, `thinkers/thk-kirk-hautvast.md`, `thinkers/thk-erica-cochrane.md`, `thinkers/thk-penelope-eckert.md` ×2 including `primary_school`, `thinkers/thk-sali-tagliamonte.md`). Deleted `sch-sociolinguistics-variation.md`.

### Additional en:-value duplicate scan (instruments/ + thinkers/)

Scanned all files in `content/ar/instruments/` and `content/ar/thinkers/` for matching `en:` values (case/punctuation-insensitive). Found 50 clear same-entity groups (49 real people/instruments + excluded 1 false group of 3 unrelated "unverified identity" stub files that only coincidentally shared placeholder `en:` text). All 50 were genuine duplicates: in every case one file was a full, well-developed article and the other a near-empty auto-generated template stub (~370–750 chars) or, in ~19 groups, both files carried substantial independent content.

Canonical choice per group: the slug with more inbound `related`/`edges` references (ties broken toward the fuller/better-named file). For the 19 groups where the losing file also had substantial unique content (≥1000 chars), its body was merged into the canonical file (appended before the sources section, marked with an inline merge-date comment) rather than discarded; the other 31 groups' losing files were pure template stubs with nothing worth preserving, so they were deleted outright after repointing.

Merged with unique content preserved: `thk-besselvanderkolk`←`thk-bessel-van-der-kolk`, `thk-rkthaler`←`thk-richard-thaler`, `thk-william-labov`←`thk-labov-variationist`, `thk-ellis`←`thk-albert-ellis`, `thk-graeber`←`thk-david-graeber`, `thk-bourdieu`←`thk-pierre-bourdieu`, `thk-appadurai`←`thk-arjun-appadurai`, `thk-winnicott`←`thk-donald-winnicott`, `thk-quentin-meillassoux`←`thk-meillassoux`, `thk-lindatuhiwai`←`thk-linda-tuhiwai-smith`, `thk-kernberg`←`thk-otto-kernberg`, `thk-janet`←`thk-pierre-janet`, `thk-gbateson`←`thk-gregory-bateson`, `thk-nawal-el-saadawi`←`thk-nawal-elsaadawi`, `thk-rogers`←`thk-carl-rogers`, `thk-afghani`←`thk-jamal-al-afghani`, `thk-chodorow`←`thk-nancy-chodorow`, `thk-freud`←`thk-sigmund-freud`, `thk-piaget`←`thk-jean-piaget`.

Stub deletions (no unique content, repointed only): `ins-multi-sited-ethnography`←`ins-multisited-ethnography`; `thk-susan-gal`←`thk-gal-language-ideology`; `thk-gregory-hickok`←`thk-hickok-dorsal-ventral`; `thk-george-marcus`←`thk-george-marcus-bio`; `thk-carol-myers-scotton`←`thk-myers-scotton-matrix`; `thk-deborah-cameron`←`thk-cameron-language-power`,`thk-cameron-language-society`; `thk-ainsworth`←`thk-mary-ainsworth`; `thk-sidney-mintz`←`thk-diasporas-scholars-mintz`; `thk-edmund-leach`←`thk-edmund-leach-biography`; `thk-michael-silverstein`←`thk-silverstein-ethnography`; `thk-bernard-comrie`←`thk-comrie-language-universals`; `thk-ofelia-garcia`←`thk-garcia-bilingual-education`; `thk-lesley-milroy`←`thk-milroy-network-dynamics`; `thk-lindatuhiwai`←`thk-linda-smith-indigenous`; `thk-penelope-gardner-chloros`←`thk-gardner-chloros-codeswitching`; `thk-paul-hopper`←`thk-hopper-emergent-grammar`; `thk-david-poeppel`←`thk-poeppel-temporal-dynamics`; `thk-joseph-greenberg`←`thk-greenberg-linguistic-universals`; `thk-pieter-muysken`←`thk-muysken-code-mixing`; `thk-dell-hymes`←`thk-hymes-ethnography-communication`; `thk-michael-tomasello`←`thk-tomasello-social-origins`; `thk-nancy-scheper-hughes`←`thk-nancy-scheper-hughes-bio`; `thk-norman-fairclough`←`thk-fairclough-critical-discourse`; `thk-crenshaw`←`thk-kimberly-crenshaw`; `thk-ward-goodenough`←`thk-ward-goodenough-bio`; `thk-damasio`←`thk-damasio-emotion-language`; `thk-tove-skutnabb-kangas`←`thk-skutnabb-kangas-rights`; `thk-rajeswari-sunder-rajan`←`rajeshwari-sunder-rajan` (unprefixed-slug stub); `thk-sahmed`←`thk-sarah-ahmed`; `thk-george-lakoff`←`thk-lakoff-politeness`,`thk-lakoff-metaphor`; `thk-noam-chomsky`←`thk-chomsky-universal-grammar`; `thk-haraway`←`thk-donna-haraway`.

Excluded from merging: `thk-cwhitaker-pt`/`thk-jgould`/`thk-jjoyce` — these only matched on the placeholder `en: "Unverified identity — see gaps"` value, not on being the same person; left untouched.

Repointed every `related`/`edges`/`primary_school` reference across `content/ar` from each deleted slug to its canonical replacement (53 losing files total across 50 groups, including one late catch — `concepts/con-psychoanalytic-theory.md`'s two `related` entries for `thk-donald-winnicott`/`thk-otto-kernberg`, missed by the initial repoint pass and fixed after the checker flagged it).
