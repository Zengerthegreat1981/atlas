...
## ملحق: ملفّ يصف كتاباً مختلفاً عن كتابه الحقيقي (ليس ازدواجاً، مسألة هُويّة)

`wrk-wertz-five-ways-phenomenological-analysis` — الملف الموروث وصف الكتاب بأنه يقارن
**خمس مدارس فينومينولوجية فرعية**. والكتاب الحقيقي (ويرتز، *Five Ways of Doing
Qualitative Analysis*، Guilford 2011) يقارن **خمس مناهج نوعية**، **واحد منها فقط
فينومينولوجي** (فيرتز نفسه)؛ الأربعة الباقية: النظرية المؤسَّسة (شارماز)، تحليل الخطاب
(ماك مولين)، البحث السردي (جوسلسون)، الاستقصاء الحدسي (أندرسون). تحقّق الوكيل من هذا
مقابل `content/ar/thinkers/thk-wertz.md` نفسه في الأطلس، الذي يوثّق الصيغة الصحيحة.

**لم يُعدَّل `slug`/`title`/`en`** (تصف الإطار الخاطئ حرفياً) لأن ذلك خارج تفويض الوكيل
وقد يكسر روابط. المتن صُحِّح إلى الوصف الحقيقي والتناقض سُجِّل في `gaps`. **يحتاج قرار
تسمية**: إمّا تصحيح `title`/`en`/`crumb` لتطابق الكتاب الحقيقي، أو نقل الـslug.

---

# تصحيح شامل 2026-09-03 — العدّ النهائي: 40 مجموعة، 82 ملفاً

## ⚠️ ما حدث: إضافات بلا مصالحة

بعد «القياس السابع» (36 مجموعة، 73 ملفاً) واصلتُ إضافة مجموعات **37** و**38** بأرقام
متسلسلة عند كل اكتشاف جديد من وكيل، **بلا التحقّق من أنها ليست موجودة أصلاً** ضمن الـ36.
والنتيجة: **المجموعة 38 (ميرلوبونتي، `...-of-...` مقابل `...perception-merleau-ponty`)
كانت أصلاً أحد زوجَي ملفّ ميرلوبونتي الثلاثة**، والملفّ الثالث (`wrk-merleau-ponty-
phenomenology-perception`) موثَّق منذ الجدول الأصلي الأول في هذا الملف (السطر 37) —
أي أن «المجموعة 38» **لم تكن جديدة، بل إعادة اكتشاف لِما هو محسوب أصلاً**. وهذا **العاشر**
من نمط «قياس بلا فحص ما هو موجود أصلاً».

## القياس النهائي — دُمجت كل الإشارات في كشّاف واحد وأُعيد كل شيء من الصفر

استُعمل **اتحاد** ثلاث علامات: (أ) تطابق `en:` الحرفي، (ب) نفس `author_slug` ونفس
`publication_year` مع تراكب 50%+ من كلمات العنوان أو الـslug، (جـ) لمؤلِّفين معروفين
بحزم أعمال متمايزة بسنة تقديرية واحدة (أرسطو، ابن رشد، فخر الرازي) — شرطٌ أشدّ (تراكب
80%+ في كلمات الـslug) لتفادي دمج كتب مختلفة فعلاً.

**ثمانية إيجابيات كاذبة استُبعدت يدوياً** بعد فحصها فرداً فرداً: مقولات أرسطو الأربع
(الطبيعة/الميتافيزيقا، والتحليلات الأولى/المقولات/عن النفس، والشعر/الخطابة)، الأمير
مقابل الخلاصة الفارابية (لا — لم تظهر)، ابن رشد (فصل المقال مقابل الكشف عن مناهج
الأدلّة — **كتابان متمايزان تماماً**، تحقّق منه وكيلٌ في 6.7 مباشرةً)، كانط
(ميتافيزيقا الأخلاق 1797 مقابل تأسيسها 1785)، شوارتز (كتاب 2021 الشعبي مقابل
الكتاب الأكاديمي 1995 — لا حقّاً بل اللاحق كتاب آخر لا نسخة)، كيركغور (إمّا/أو
مقابل خوف ورعدة — كتابان)، ابن سينا (النجاة مقابل الشفاء — مؤكَّد سابقاً)، أفلاطون
(الدفاع مقابل أقريطون)، **والرازي (الأربعين في أصول الدين مقابل محصل أفكار
المتقدمين — كتابان متمايزان تماماً، اكتُشف في هذا التصحيح نفسه)**.

## العدد النهائي المعتمَد

| | العدد |
|---|---|
| **مجموعات مؤكَّدة** | **40** |
| **ملفات متأثِّرة** | **82** |
| مجموعات من ملفَّين | 38 |
| مجموعات من ثلاثة ملفات | 2 (سارتر، ميرلوبونتي) |

**هذا هو الرقم المعتمَد من الآن فصاعداً، ويُلغي كل رقم سابق في هذا الملف (32، 36،
37، 38) — لا لأن تلك القياسات كانت خاطئة فرداً فرداً، بل لأنها لم تُصالَح مع بعضها.**
القائمة الكاملة بالأطوال محفوظة في `scratchpad/final_dup_groups.json` لمرجعية المحرِّر.

## الدرس — العاشر من نفس النوع، وهو الأهمّ

إضافة رقم متسلسل عند كل اكتشاف بلا إعادة حساب الإجمالي هي **نفس خطأ القياس الجزئي**
الذي ارتُكب تسع مرّات سابقاً، لكن في صورة جديدة: هنا لم يكن الحقل أو الأداة مقيسة
خطأً، بل **طريقة العدّ التراكمي نفسها** كانت الخطأ. من الآن: أيّ اكتشاف جديد
**يُدمَج في القياس الشامل ويُعاد حساب الإجمالي كاملاً**، لا يُضاف رقمه إلى آخر رقم مذكور.

---

## تصحيح فوري: الرقم 40/82 كان ناقصاً — `wrk-plato-republic` أُغفل

بعد نشر «العدّ النهائي» مباشرةً، سحبت دفعة 6.9 `wrk-republic-plato` فوجدتُ أن
الزوج المعروف منذ الدفعة الأولى (`wrk-plato-republic` ⇄ `wrk-republic-plato`)
**غير موجود في القائمة النهائية 40/82** — لأن `wrk-plato-republic` **بلا
`author_slug` ولا `publication_year` إطلاقاً**، فسقط من كشّاف الاتحاد الذي بنيتُه
على هذين الحقلَين + `en:`. وهذه غفلة حدثت في نفس التصحيح الذي قصد إصلاح غفلات
مشابهة — **الحادية عشرة من نفس النوع**.

**العدد الصحيح النهائي: 41 مجموعة، 84 ملفاً.**

الدرس المضاف: كشّاف مبنيّ على حقلَين قد يُغفل زوجاً حين **يفتقر أحد طرفيه للحقلَين
معاً** — وهذا بالضبط ما يميّز الملفات الأضعف توثيقاً، أي الأكثر احتياجاً للرصد لا الأقل.
من الآن: أي كشّاف مستقبلي يضيف **فحص تراكب العنوان وحده** (بلا شرط الحقول) كطبقة
ثالثة، لا بديلاً بل إضافة.

## إضافة — المجموعة 42: نقد الفكر الديني للعظم (وجدها سحب 6.16، قبل التعميق)

`wrk-alazm-critique-religious-thought` (WRK-0225, عُمِّق في 6.15) ⇄
`wrk-naqd-al-fikr-al-dini-al-azm` (WRK-0644, لم يُعمَّق بعد وقت الاكتشاف) —
نفس المؤلف (صادق جلال العظم)، نفس السنة (1969)، نفس الكتاب فعلياً
(العنوان الثاني نقحرة/ترجمة حرفية للأول). لم يظهر في أي كشّاف اتحاد سابق
لأن الأول عنوانه إنجليزي معرَّب (`alazm-critique-religious-thought`)
والثاني عنوانه عربي منقول بالحروف اللاتينية (`naqd-al-fikr-al-dini-al-azm`)
— **نمط "لا كلمة مشتركة" الرابع من الأنماط الأربعة المسجَّلة أصلاً**، لكنه
فات كشّاف الاتحاد لأن تراكب التوكِن بين الاسمين تقريباً صفر.

**العدد المحدَّث: 42 مجموعة، 86 ملفاً.**

القرار التنفيذي (اتّساقاً مع نمط 6.9/6.11): يُعمَّق `wrk-naqd-al-fikr-al-dini-al-azm`
باستقلالية (نسخة مختلفة الصياغة عن الطرف الآخر المعمَّق فعلاً)، لا يُدمَج ولا يُحذَف،
ويُسجَّل الازدواج صراحةً في `gaps` كلا الملفين.

## إضافة — المجموعة 43: حكم/تعاليم بتاح حتب (وجدها وكيل في 6.17)

`wrk-maxims-ptahhotep` (WRK-0631, عُمِّق في 6.17) ⇄ `wrk-instructions-ptahhotep`
(WRK-0359) — نفس المؤلف (بتاح حتب)، نفس السنة التقديرية (-2400)، نفس العمل
المصري القديم بعنوانين مختلفين (حكم/تعاليم). لم يظهر في كشّاف الاتحاد السابق
لتراكب توكِن ضعيف بين "maxims" و"instructions" — نمط "لا كلمة مشتركة" مرة أخرى
(مثل المجموعة 42).

**العدد المحدَّث: 43 مجموعة، 88 ملفاً.**

القرار التنفيذي: عُمِّق `wrk-maxims-ptahhotep` باستقلالية (لم يُنسَخ من الطرف
الآخر)، لم يُدمَج، لم يُحذَف. سُجِّل الازدواج في `gaps`.

## إضافة — المجموعة 44: تفسير الأحلام لفرويد (وجدها سحب 6.18، قبل التعميق)

`wrk-die-traumdeutung` (WRK-0281, في هذه الدفعة) ⇄ `wrk-freud-interpretation-of-dreams`
(WRK-0772, عُمِّق فعلاً في 6.15) — نفس المؤلف فرويد، نفس الكتاب (1899/1900،
العنوان الألماني مقابل العنوان الإنجليزي المترجَم)، نفس السنة تقريباً (فارق توثيق
1899/1900 معروف أصلاً بين النسختين). نمط "لا كلمة مشتركة" (الألمانية مقابل
الإنجليزية) — نفس نمط المجموعتين 42 و43.

**العدد المحدَّث: 44 مجموعة، 90 ملفاً.**

القرار التنفيذي: يُعمَّق `wrk-die-traumdeutung` باستقلالية (لا نسخ من الطرف الآخر)،
لا يُدمَج، لا يُحذَف. يُسجَّل الازدواج صراحة في `gaps` كلا الملفين.

## إضافة — المجموعة 45: المرض حتى الموت لكيركيغارد (وجدها وكيل في 6.18)

`wrk-sickness-unto-death-kierkegaard` (WRK-0695, عُمِّق في 6.18) ⇄ `wrk-sickness-death`
(WRK-0203, معمَّق سابقاً) — نفس المؤلف كيركيغارد، نفس السنة 1849، نفس الكتاب
بصياغتي slug مختلفتين (عنوان إنجليزي+اسم المؤلف مقابل عنوان عربي مختصر).

**العدد المحدَّث: 45 مجموعة، 92 ملفاً.**

القرار التنفيذي: عُمِّق `wrk-sickness-unto-death-kierkegaard` باستقلالية (لا نسخ)،
لا دمج، لا حذف. سُجِّل الازدواج في `gaps`.

## إضافة — المجموعة 46: الاستشراق لإدوارد سعيد (وجدها وكيل في 6.21)

`wrk-orientalism-said` (WRK-0414, عُمِّق في 6.21) ⇄ `wrk-orientalism-edward-said`
(WRK-0661, معمَّق سابقاً، أطول وبترجمتين عربيتين موثَّقتين) — نفس المؤلف
والكتاب والسنة (1978). كان مذكوراً جزئياً فقط في `agents_specs/requests-minimax.md`
(ملاحظة Task 11 حول `crt-said-orientalism-epistemic-critique`) لكن لم يُسجَّل رسمياً
هنا من قبل.

**العدد المحدَّث: 46 مجموعة، 94 ملفاً.**

القرار التنفيذي: عُمِّق `wrk-orientalism-said` باستقلالية (لا نسخ)، لا دمج، لا حذف.
سُجِّل الازدواج في `gaps`.

## إضافة — المجموعة 47: إشكالية النوع لجوديث بتلر (وجدها وكيل في Task 4.11)

`wrk-gender-trouble` (WRK-0322) ⇄ `wrk-gender-trouble-butler` (WRK-0597) —
نفس المؤلفة (جوديث بتلر)، نفس الكتاب (Gender Trouble، 1990)، نفس السنة.
اكتُشف أثناء تعميق ملف المفكرة `thk-butler` في Task 4 (لا Task 6)، خارج
مسح دفعات works/ الأصلي.

**العدد المحدَّث: 47 مجموعة، 96 ملفاً.**

القرار: لم يُحسم دمج أو حذف — سُجِّل الازدواج في `gaps` كلا الملفين، بانتظار
قرار تحرير مركزي، طبقاً لنفس البروتوكول القائم لكل الازدواجات الأخرى.
