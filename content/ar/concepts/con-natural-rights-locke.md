---
slug: "con-natural-rights-locke"
id: "CON-1091"
type: "مفهوم"
part: "philosophy"
level: "متقدم"
title: "الحقوق الطبيعية الأصيلة للإنسان (Natural Rights)"
en: "Natural Rights Locke"
crumb: "المفاهيم الفلسفية الكبرى ← الحقوق الطبيعية الأصيلة للإنسان (Natural Rights)"
active_start: 1689
active_end: "مستمر"
edges:
- rel: "belongs_to", target: "sch-british-empiricism", target_type: "مدرسة"
related:
- id: "thk-locke", title: "جون لوك", type: "مفكر"
- id: "sch-british-empiricism", title: "التجريبية البريطانية (كمظلة)", type: "مدرسة"
- id: "wrk-two-treatises-of-government-locke", title: "رسالتان في الحكم المدني لجون لوك", type: "عمل / كتاب"
- id: "con-social-contract-theory", title: "نظرية العقد الاجتماعي (Social Contract)", type: "مفهوم"
- id: "con-negative-liberty-berlin", title: "الحرية السلبية وغياب القيود القسرية (Negative Liberty)", type: "مفهوم"
gaps:
  - "التحليل الإبستمولوجي واللغوي الدقيق لتطور المفهوم عبر التاريخ."
  - "قسمُ «المصادر» في هذا الملفّ كان يسرد عناوينَ مداخلَ أخرى من الأطلس نفسِه مأخوذةً من حقل `related` — أي إحالاتٍ داخليةً لا مصادرَ خارجية. غُيِّر عنوانُ القسم إلى ما يصفه، والملفُّ ما زال بلا مصادرَ موثَّقةٍ خارجية."
---
# الحقوق الطبيعية الأصيلة للإنسان (Natural Rights)

الحقوق اللصيقة بكرامة الإنسان التي لا يجوز للدولة انتهاكها: الحياة، الحرية، والملكية.

## الروابط والسياق

صاغ **جون لوك** (`thk-locke`) هذا المفهوم داخل مدرسته الفكرية **التجريبية البريطانية** (`sch-british-empiricism`)، وفصّله في كتابه **«رسالتان في الحكم المدني»** (`wrk-two-treatises-of-government-locke`). ترتبط الحقوق الطبيعية عضوياً بـ**نظرية العقد الاجتماعي** (`con-social-contract-theory`) التي تبرر قيام الدولة على أساس حماية هذه الحقوق، وتتقاطع نقدياً مع مفهوم **الحرية السلبية عند برلين** (`con-negative-liberty-berlin`) بوصفه إعادة صياغة القرن العشرين لفكرة الفضاء الفردي المحمي من تدخل الدولة.

## مداخلُ الأطلس المتّصلة

مداخلُ الأطلس المتّصلةُ بهذا المدخل (من حقل `related`) — وهي إحالاتٌ داخلية، لا مصادرُ خارجية:

- *رسالتان في الحكم المدني لجون لوك*
