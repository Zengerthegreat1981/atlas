---
slug: "thk-rvendramini"
id: "THK-1885"
type: "مفكر"
level: "مبتدئ"
title: "رِيناتا فِندْراميني"
en: "Renata Vendramini"
dates: "إيطاليا"
country: "إيطاليا"
language: "الإيطالية"
active_start: 1990
active_end: "مستمر"
part: "psychology"
crumb: "التحليل النفسي ← الناس ← رِيناتا فِندْراميني"
edges:
- rel: "belongs_to", target: "sch-analytical-psychology", target_type: "مدرسة"
related:
- id: "thk-zoja", title: "لويجي تسويا", type: "مفكر"
- id: "thk-meharding", title: "ماري إستر هاردينغ", type: "مفكر"
- id: "thk-twolff", title: "توني فولف", type: "مفكر"
- id: "thk-jung", title: "كارل غوستاف يونغ", type: "مفكر"
gaps:
  - "لا توجد سيرة ذاتية منشورة لـ 'Renata Vendramini' في AIPA (Associazione Italiana di Psicologia Analitica) حتى آب 2026."
  - "الكتاب المشار إليه ('La Femminilità Ritrovata', التسعينيات) لا يظهر في SBN (Biblioteca Nazionale Italiana)."
  - "الاحتمال: خلط مع 'Renata V. Salerno' (محلِّلة يونغية في إيطاليا) أو 'Renata Taddei' (محلِّلة يونغية في ميلانو)."
  - "يحتاج قرار من رئيس التحرير: حجر، تصحيح، أو دمج في ملف 'Jungian Italia' عام."
  - "**وُحِّد مسارُ التنقّل 2026-09-10:** كان «مدرسة التحليل النفسي» وصار «التحليل النفسي» — لأنّ الجذرَ كان منقسماً بين صيغتين للشيء نفسِه، فيعرض فهرسُ التصفّح موضعين لموضعٍ واحد."
  - "**صُحِّح `part` 2026-09-10:** كان «philosophy» تبعاً لأبٍ كان موسوماً «philosophy» خطأً؛ وصُحِّح الأبُ فصُحِّح معه."
  - "حُذف سطرُ مصدرٍ رابطُه ميّت (0): AIPA: https://www.aipa-psy.it/"
---
# رِيناتا فِندْراميني

[هذا العنصر يحتاج مراجعة — الاسم المُسجَّل غير موثَّق في التحليلية الإيطالية]

## محاولة التوثيق

- **الاسم المُسجَّل**: Renata Vendramini.
- **البحث في قواعد البيانات (حتى آب 2026)**:
  - **AIPA**: لا عضوية.
  - **SBN (Biblioteca Nazionale Italiana)**: لا كتاب منشور.
  - **Rivista di Psicologia Analitica**: لا مقالة.
  - **AIMI / IAAP (Italy)**: لا عضوية.

## السبب المحتمل للإدراج

- احتمال **الخلط** مع:
  - **Renata V. Salerno** (محلِّلة يونغية في إيطاليا).
  - **Renata Taddei** (محلِّلة يونغية في ميلانو).
  - **Mirella Galeota** (محلِّلة يونغية في نابولي، نسوية).

## ما يحتاجه هذا العنصر

- **إما**: تصحيح الاسم.
- **أو**: الحجر في `quarantine-minimax.md`.

## المصادر

- لا توجد مصادر موثّقة.
- للتحقق:
  - SBN: https://opac.sbn.it/
  - Zoja, L. (2003). *Drugs, Addiction, and Initiation*. Daimon.
