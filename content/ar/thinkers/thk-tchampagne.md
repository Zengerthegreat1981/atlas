---
slug: "thk-tchampagne"
id: "THK-2025"
type: "مفكر"
level: "مبتدئ"
title: "تينا شامبانيا"
en: "Tina Champagne"
dates: "الولايات المتحدة"
country: "الولايات المتحدة"
language: "الإنجليزية"
active_start: 1995
active_end: "مستمر"
part: "psychology"
crumb: "التكامل الحسي ← تطوير ← تينا شامبانيا"
edges:
- rel: "belongs_to", target: "br-sensory-integration", target_type: "تيار"
related:
- id: "thk-jayres", title: "جين أَيرس", type: "مفكر"
- id: "thk-rschaaf", title: "روزان شاف", type: "مفكر"
- id: "thk-lmiller", title: "لوسي جين ميلر", type: "مفكر"
gaps:
  - "لا توجد سيرة ذاتية منشورة لـ 'Tina Champagne' في WPS أو USC Chan (حتى آب 2026)."
  - "الاحتمال: الاسم مُدخل من بيانات ممارسي OT في خدمات الصحة النفسية."
  - "يحتاج قرار من رئيس التحرير: حجر، تصحيح، أو دمج في ملف 'Sensory Modulation' عام."
  - "**صُحِّح `part` 2026-09-10:** كان «philosophy» بينما أبوه `br-sensory-integration` موسومٌ «psychology» — فوُحِّد على وسم الأب. والوسمُ يحكم التصفيةَ والفهارسَ في الواجهة، فاختلافُه عن الأب يُخرج العقدةَ من فهرس قسمها."
---

# تينا شامبانيا

[هذا العنصر يحتاج مراجعة — الاسم المُسجَّل غير موثَّق في التكامل الحسي]

## محاولة التوثيق

- **الاسم المُسجَّل**: Tina Champagne.
- **البحث في قواعد البيانات (حتى آب 2026)**:
  - **WPS (Western Psychological Services)**: لا سيرة.
  - **USC Chan Sensory Integration**: لا نتائج.
  - **AOTA (American Occupational Therapy Association)**: لا نتائج.

## السبب المحتمل للإدراج

- احتمال **خلط** مع **Tina Champagne** (ممارس OT في خدمات الصحة النفسية، الشخصية حقيقية).

## ما يحتاجه هذا العنصر

- **إما**: تصحيح الاسم.
- **أو**: الحجر في `quarantine-minimax.md`.

## المصادر

- لا توجد مصادر موثّقة.
- للتحقق:
  - Ayres, A. J. (1972). *Sensory Integration and Learning Disorders*. WPS.
  - Champagne, T. (2011). «Sensory Modulation & Environment: Essential Elements of Occupation» (Handout & Presentations). Massachusetts General Hospital.
  - Champagne, T. & Stromberg, N. (2004). «Sensory Modulation for Inpatient Psychiatry», *American Occupational Therapy Association Mental Health Special Interest Section Quarterly* 27(2): 1–4.
