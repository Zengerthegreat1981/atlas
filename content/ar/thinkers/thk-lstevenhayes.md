---
slug: "thk-lstevenhayes"
id: "THK-2432"
type: "مفكر"
part: "psychology"
level: "متقدم"
title: "ستيفن سي. هايز"
en: "Steven C. Hayes"
crumb: "علم النفس ← العلاج بالقبول والالتزام (ACT) ← مؤسس ← ستيفن هايز"
dates: "الولايات المتحدة · 1948–حتى الآن"
active_start: 1982
active_end: "مستمر"
country: "الولايات المتحدة"
language: "الإنجليزية"
register: "academic"
evidence_level: "well-established"
edges:
- rel: "belongs_to", target: "العلاج بالقبول والالتزام (ACT)", target_type: "مدرسة"
related:
- id: "rel-integrative-cbt-common-factors", title: "التكاملية (العامل المشترك) والعلاج المعرفي-السلوكي: تحدّي الصلة", type: "علاقة بين مدرستين"
- id: "rel-buddhism-thirdwave", title: "البوذية والموجة الثالثة من العلاجات: استيراد اليقظة من فيباسانا إلى MBSR وACT وDBT", type: "علاقة بين مدرستين"
- id: "tec-acceptance-based-bt", title: "العلاج السلوكي القائم على القبول (ABBT)", type: "تقنية/تدخل علاجي"
- id: "tec-act-hexaflex", title: "العلاج بالقبول والالتزام (ACT) — هيكسافلكس", type: "تقنية/تدخل علاجي"
- id: "tec-process-based-cbt", title: "العلاج المعرفي السلوكي القائم على العمليات", type: "تقنية/تدخل علاجي"
- id: "br-functional-contextualism-rft", title: "السياقية الوظيفية ونظرية إطار العلاقات (RFT)", type: "تيار"
- id: "sch-act", title: "العلاج بالقبول والالتزام", type: "مدرسة"
- id: "sch-transpersonal", title: "علم النفس عبر الشخصي", type: "مدرسة"
- id: "con-relational-frame-theory", title: "نظرية الإطار العلائقي", type: "مفهوم"
- id: "tec-cbt-cog-thought-defusion", title: "الفصل المعرفي", type: "تقنية/تدخل علاجي"
- id: "tec-act-ca-behavioral-commitment-exercises", title: "الفعل الملتزم", type: "تقنية/تدخل علاجي"
- id: "dbt-cbt-third-wave-vs-second-wave", title: "جِدَل الْمَوْجَة الثَّالِثَة لِـ**CBT** ضِد الثَّانِيّة: تَطَوُّر أَم انْفِصَال؟", type: "جدل"
gaps:
  - "رابط مقترح لم يتحقق بعد في الأطلس: كيلي ستودارد (thk-kstoddard)"
  - "رابط مقترح لم يتحقق بعد في الأطلس: روني باير (thk-rbaer)"
  - "رابط مقترح لم يتحقق بعد في الأطلس: كيتلين ويلسون (thk-kwilson)"
  - "رابط مقترح لم يتحقق بعد في الأطلس: ACT (con-acceptance-commitment)"
  - "رابط مقترح لم يتحقق بعد في الأطلس: المرونة النفسية (con-psychological-flexibility)"
  - "رابط مقترح لم يتحقق بعد في الأطلس: العمليات الست (Hexaflex) (con-six-processes)"
  - "رابط مقترح لم يتحقق بعد في الأطلس: توضيح القيم (con-values-clarification)"
  - "تفاصيل تجربته الشخصية مع المرض (panic disorder) التي دفعته لتطوير ACT موثّقة جزئياً."
---

# ستيفن سي. هايز (Steven C. Hayes)

عالم نفس أمريكي، مؤسّس **العلاج بالقبول والالتزام** (Acceptance and Commitment Therapy, ACT) في الثمانينيات، ومطوّر **نظرية الإطار العلائقي** (Relational Frame Theory, RFT)، الأسس النظرية لـ ACT. أستاذ جامعي ومنتج (50+ كتاب، 600+ مقال).

## ما أعطاه

### 1. نظرية الإطار العلائقي (Relational Frame Theory, RFT)

طوّر هايز RFT ك**نظرية للغة والإدراك البشري**:
- **"الإطار العلائقي"** (Relational Framing): قدرة الإنسان على ربط أي شيء بأي شيء (مثل: "السحابة تشبه البطانية").
- هذا ما يُميّز اللغة من السلوك الحيواني.
- **التعلم العلائقي** (Relational Learning) يفسر كيف نتعلم الكلمات.
- RFT تُشكّل الأساس لـ ACT.

### 2. ACT (Acceptance and Commitment Therapy, الثمانينيات من القرن العشرين-)

طوّر هايز ACT كعلاج "الجيل الثالث" من العلاج السلوكي المعرفي (CBT):
- **الفلسفة**: المعاناة الإنسانية ليست من "الأفكار السلبية"، بل من **محاولة التخلص** منها.
- **الهدف**: **المرونة النفسية** (Psychological Flexibility) — القدرة على الفعل وفق القيم حتى في حضور المشاعر الصعبة.
- **6 عمليات أساسية (Hexaflex)**:
  1. **القبول (Acceptance)**: تقبّل المشاعر كما هي.
  2. **الفصل المعرفي (Cognitive Defusion)**: رؤية الأفكار كأفكار، ليست "حقيقة".
  3. **التماس اللحظة (Present Moment Awareness)**: العيش في الحاضر.
  4. **الذات كسياق (Self-as-Context)**: "أنا" أكبر من أفكاري.
  5. **القيم (Values)**: ما يهم الفرد حقاً.
  6. **الفعل الملتزم (Committed Action)**: الفعل وفق القيم.

### 3. علاقته بـ CBT التقليدية

- **CBT التقليدية**: "غيّر أفكارك السلبية".
- **ACT**: "اقبل أفكارك السلبية، واعمل وفق قيمك".
- ACT ليست "مضادة" لـ CBT، بل **مكمّلة**، ضمن "الموجة الثالثة" من العلاجات السلوكية (Linehan، Marlatt، Hayes).

### 4. تطبيقاته الواسعة

- **ACT** لـ **القلق، الاكتئاب، PTSD، OCD، اضطرابات الأكل، الإدمان**.
- **ACT في الطب**: للأمراض المزمنة، الألم المزمن.
- **ACT في المنظمات**: القيادة، الأداء.
- **ACT في التعليم**: تربية الأطفال.
- **ACT قصصي (ACT Matrix)**: تبسيط للممارسين.

## موقعه من التيار

- **مقابل CBT**: مقاربة "الموجة الثالثة"، تُقبل بدلاً من "تغيير" الأفكار.
- **مقابل MBSR/MBCT**: ACT تختلف في تركيزها على "القيم" و"الفعل"، لا فقط "الوعي".
- **مقابل فرويد**: لا "اللاوعي"، بل "تأثير اللغة".
- **مقابل علم النفس البوذي**: ACT تأثرت بها، لكن "علمية" (Evidence-Based) أكثر.

## أهم أعماله

- **"العلاج بالقبول والالتزام" (Acceptance and Commitment Therapy, 1999)** — مع Strosahl & Wilson.
- **"نظرية الإطار العلائقي: نهج سلوكي للغة البشرية" (Relational Frame Theory, 2001)** — مع Dymond.
- **"هدم جدار العار" (Liberating the Life of Buddha, 2010)** — مع McCurry.
- **"ACT Made Simple" (2009, 2019)** — مع Forsyth.
- **"العقل المحرَّر" (A Liberated Mind, 2019)** — كتاب شعبي.

## الأبحاث

- **Hayes et al. (1999)**: "العلاج بالقبول والالتزام: نموذج تجريبي للاضطرابات السلوكية."
- **Hayes et al. (2006)**: "نظرية الإطار العلائقي: نظرية للغة والسلوك."
- **A-Tjak et al. (2015)**: تحليل ميتا-تحليلي لـ ACT، فعالية متوسطة.
- **Powers et al. (2009)**: ACT لـ الاكتئاب.

## الانتقادات

- **RFT**: نظرية معقدة، قد لا تكون ضرورية لفهم ACT.
- **"القبول"** كاستراتيجية: قد يبدو "استسلامياً" في حالات الإساءة.
- **الحركة الشعبية**: ACT "أصبحت تجارية" مع عدد كبير من المعالجين "المعتمدين".
- **الفجوة بين البحث والسريري**: كثير من الدراسات صغيرة.

## الأهمية للأطلس

- واحد من أكثر العلاجات تأثيراً في القرن الحادي والعشرين.
- يجمع بين علم النفس السلوكي، الفلسفة البوذية، ونظرية اللغة.
- إطار للدمج بين العلاج والممارسة الروحية.

## المرجع الأساسي

- Hayes, S. C., Strosahl, K. D., & Wilson, K. G. (1999). *Acceptance and Commitment Therapy: An Experiential Approach to Behavior Change*. Guilford Press.
- Hayes, S. C., Barnes-Holmes, D., & Roche, B. (Eds.). (2001). *Relational Frame Theory: A Post-Skinnerian Account of Human Language and Cognition*. Plenum.
- Hayes, S. C., & Smith, S. (2005). *Get Out of Your Mind and Into Your Life: The New Acceptance and Commitment Therapy*. New Harbinger.
- Hayes, S. C. (2004). "Acceptance and commitment therapy, relational frame theory, and the third wave of behavioral and cognitive therapies." *Behavior Therapy*, 35(4), 639-665.
- Hayes, S. C. (2019). *A Liberated Mind: The Essential Guide to ACT*. Avery.

---

## ملاحظة معمارية

هذا الملف يحل محل ملف thk-stevenhayes (THK-...) الموجود في `content/ar/thinkers/` — الهدف ترقية المسودة للموضع المعتمد بعد مراجعة وكيل 4.
