---
slug: "thk-rollnick"
id: "THK-1851"
type: "مفكر"
level: "مبتدئ"
title: "ستيفن رولنيك"
en: "Stephen Rollnick"
crumb: "الإنسانية ← الناس ← ستيفن رولنيك"
dates: "بريطانيا (ويلز) · 1950–"
active_start: 1983
active_end: "مستمر"
country: "المملكة المتحدة (ويلز)"
language: "الإنجليزية"
part: "psychology"
edges:
- rel: "belongs_to", target: "علم النفس الإنساني (Humanistic Psychology)", target_type: "مدرسة"
- rel: "co-developed", target: "المقابلة التحفيزية (Motivational Interviewing)", target_type: "تيار"
related:
- id: "tec-motivational-interviewing", title: "المقابلات الدافعية (MI)", type: "تقنية/تدخل علاجي"
- id: "br-psychodynamic-humanistic", title: "علم النفس الإنساني العميق (رولو ماي)", type: "تيار"
- id: "thk-assagioli", title: "روبرتو أساجيولي", type: "مفكر"
- id: "thk-dabrowski", title: "كازيمير دابروفسكي", type: "مفكر"
- id: "thk-farrelly", title: "فرانك فاريلي", type: "مفكر"
- id: "thk-lieberman", title: "ألسي ليبرمان (باتريسيا)", type: "مفكر"
- id: "con-motivational-spirit", title: "روح المقابلة التحفيزية (MI Spirit)", type: "مفهوم"
gaps:
  - "لا يوجد اقتباس مباشر موثوق متاح بعد لهذا المفكر."
  - "تاريخ الميلاد الدقيق (1950) تقريبي ولم يُراجع من مصدر أولي."
  - "لا يوجد عنصر thk-wmiller في الفهرس (ويليام ميلر المؤسس المشارك) — يحتاج إنشاء في معالجة لاحقة."
---

# ستيفن رولنيك

عالم نفس ويلزي ومتخصص في علم النفس الصحي، شارك ويليام ميلر في تأسيس «المقابلة التحفيزية» (Motivational Interviewing, MI) عام 1991، التقنية العلاجية القصيرة التي أحدثت ثورة في علاج الإدمان والالتزام بالعلاج في الطب السلوكي.

## ما أعطاه

ميّز رولنيك المقابلة التحفيزية عن الأساليب المواجهة التقليدية في علاج الإدمان، رافضاً مقاربة «الصدمة الحقيقية» (Shock Tactics) ومؤكّداً أن الدافعية للتغيير ليست سمة ثابتة في الشخص بل هي عملية يمكن استثارتها من خلال التواصل العلاجي. طوّر مع ميلر «روح المقابلة التحفيزية» (MI Spirit): الشراكة (Partnership)، التعاطف (Compassion)، الاستحضار (Evocation)، القبول (Acceptance).

توسّع رولنيك لاحقاً في تطبيق MI خارج علاج الإدمان: السكري، الصحة العامة، السلوك الإجرامي، وحتى التحفيز في الرياضة. كما طوّر نسخة مختصرة هي «العلاج التحفيزي المعزِّز» (Motivational Enhancement Therapy, MET).

## موقعه من التيار

- أقرب رواد الإنسانية إلى الممارسة السريرية الموجزة (Brief Intervention) المبنية على الأدلة.
- يُعتبر من «أبناء كارل روجرز» (Client-centered tradition)، إذ التزم بالشروط العلاجية للروجرز: التعاطف، عدم الحكم، تعزيز الدافعية الذاتية.
- أكاديمياً في جامعة كارديف (Cardiff University)، إحدى أكبر مراكز تدريب MI في العالم.

## أهم أعماله

- *Motivational Interviewing: Preparing People to Change* (1991، مع ميلر) — الكتاب المؤسس.
- *Motivational Interviewing in Health Care* (2008، مع ميلر وباتلر) — توسيع نحو الرعاية الصحية.
- *Motivational Interviewing with Adolescents* (2012، مع ناهي) — تطبيق على المراهقين.
- *Motivational Interviewing: Helping People Change* (2013، مع ميلر) — الطبعة المنقحة.

## قيد

موقعه الجغرافي في ويلز (وليس أمريكا) حدّ من حضوره في الأدبيات الأمريكية المبكرة رغم التأثير العالمي. أيضاً، يخلط كثير من الممارسين بينه وبين «ويليام ريتشارد ميلر» الأصلي (وليس W.R. Miller من جامعة نيو مكسيكو)، فيحدث التباس في الاستشهاد.
