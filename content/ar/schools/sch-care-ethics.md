---
slug: "sch-care-ethics"
id: "SCH-0055"
type: "مدرسة"
part: "philosophy"
level: "متوسط"
title: "أخلاقيات الرعاية (Care Ethics)"
en: "Care Ethics"
crumb: "الفلسفة ← المقارنة والعابرة ← أخلاقيات الرعاية"
dates: "أمريكا، عالمية · من 1982 إلى اليوم"
country: "الولايات المتحدة"
language: "الإنجليزية"
active_start: 1982
active_end: "مستمر"
cultural_origin: "anglo-american"
edges: []
related:
- id: "sch-feminism-radical", title: "النسوية الراديكالية", type: "مدرسة"
- id: "sch-virtue-ethics", title: "أخلاقيات الفضيلة الجديدة", type: "مدرسة"
- id: "wrk-different-voice", title: "صوت مختلف (In a Different Voice)", type: "عمل / كتاب"
- id: "thk-nel-noddings", title: "نيل نودينغز", type: "مفكر"
- id: "thk-gilligan", title: "كارول غيليغان", type: "مفكر"
- id: "con-care-ethics", title: "أخلاقيات الرعاية", type: "مفهوم"
- id: "con-confucian-ren", title: "الإنسانية (Ren 仁, Confucianism)", type: "مفهوم"
- id: "con-speciesism", title: "تخصيص النوع (Speciesism)", type: "مفهوم"
- id: "rel-care-ethics-relational-psychoanalysis", title: "أخلاقيات الرعاية النسوية والتحليل النفسي العلائقي (Mitchell/Gilligan)", type: "علاقة بين مدرستين"
gaps:
  - "**فُرِّغ حقلُ الانتماء 2026-09-08:** كان `belongs_to` يشير إلى `sch-comparative-philosophy` ختماً قالبياً — الفلسفةُ المقارنةُ منهجٌ ومجالٌ مستقلّ، لا مظلّةٌ للفلسفات التطبيقية المعاصرة. وهذه مدرسةُ نِدٍّ أو مظلّةٌ مستقلّة، فبقي الحقلُ فارغاً؛ ولم يُوضَع أبٌ تقريبيٌّ لأنّ أباً خاطئاً أسوأُ من غياب أب."
  - "**رُبطت عقدٌ معزولة 2026-09-08:** أُضيفت هنا إشارةٌ إلى عقدةٍ كانت لا يشير إليها أيُّ ملفّ، فكانت غيرَ قابلةٍ للوصول بالتنقّل. وكلُّ إشارةٍ عكسُ علاقةٍ يؤكّدها المستودعُ أصلاً (تأليفٌ أو انتماءٌ أو رابطٌ صادرٌ منها)، لم تُخترَع واحدةٌ منها."
  - "رابط مقترح لـ«الأخلاق النسوية» (sch-feminist-ethics) أُزيل مؤقتاً — العنصر غير موجود بعد في الأطلس (لا معتمد ولا مسودة)، يحتاج كتابة عبر مسار المسودات العادي قبل إعادة الربط."
  - "نصوص Carol Gilligan, Nel Noddings, Virginia Held, Eva Feder Kittay, Michael Slote, Emmanuel Levinas need review."
---
# أخلاقيات الرعاية (Care Ethics)
تيار أخلاقي يَنتقد المنظور الليبرالي القائم على الاستقلال والعدالة، يَؤكد على الرعاية كقيمة أخلاقية أساسية.
## الأفكار المركزية
- **In a Different Voice (صوت مختلف)**: عند Gilligan، أخلاق النساء قائم على الرعاية.
- **Responsive Care (الرعاية المستجيبة)**: عند Noddings، التعاطف كقاعدة أخلاقية.
- **The Ethics of Care (أخلاقيات الرعاية)**: عند Held، أربع ملامح (الرعاية كقيمة، كعمل، كعلاقة، كسياق).
- **Dependency (التبعية)**: عند Kittay، تقبل التبعية كحقيقة إنسانية.
- **Ethics of Responsibility (أخلاقيات المسؤولية)**: عند Levinas، مسؤولية تجاه الآخر.
## الفلاسفة
- **Carol Gilligan (كارول غيليغان، 1936-)**: «In a Different Voice» (1982).
- **Nel Noddings (نيل نودينغز، 1929-2022)**: «Caring» (1984).
- **Virginia Held (فيرجينيا هيلد)**: «The Ethics of Care» (2006).
- **Eva Feder Kittay (إيفي فِدَر كيتي)**: «Love's Labor» (1999).
- **Michael Slote (مايكل سلوت)**: «Morals from Motives» (2001).
- **Emmanuel Levinas (إيمانويل ليفيناس)**: «Totality and Infinity» (1961).
- **Sara Ruddick (سارا روديك)**: «Maternal Thinking» (1989).
## الأثر
- أثّرت في أخلاقيات الرعاية الصحية.
- أثّرت في الفلسفة السياسية.
- أثّرت في الأخلاق المهنية.
## القيد
- تواجه انتقادات بسبب التعميم النوعي.
- تواجه انتقادات بسبب إهمال البُعد العرقي.
## المصادر
- Gilligan, Carol. *In a Different Voice: Psychological Theory and Women's Development*. Harvard University Press, 1982.
- Noddings, Nel. *Caring: A Feminine Approach to Ethics and Moral Education*. University of California Press, 1984.
- Held, Virginia. *The Ethics of Care: Personal, Political, and Global*. Oxford University Press, 2006.
- Kittay, Eva Feder. *Love's Labor: Essays on Women, Equality, and Dependency*. Routledge, 1999.
