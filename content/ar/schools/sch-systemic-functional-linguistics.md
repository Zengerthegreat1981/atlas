---
slug: "sch-systemic-functional-linguistics"
id: "SCH-5201"
type: "مدرسة"
part: "linguistics"
level: "متوسط"
linguistic_level: "sociolinguistics"
cultural_origin: "anglo-european"
title: "اللسانياتُ الوظيفيةُ المنهجية (Systemic Functional Linguistics)"
en: "Systemic Functional Linguistics (SFL)"
crumb: "علم اللغة ← الوظيفية والنمطية ← اللسانياتُ الوظيفيةُ المنهجية"
dates: "بريطانيا، أستراليا · من 1961 إلى الوقت الحاضر"
country: "بريطانيا، أستراليا"
language: "الإنجليزية"
active_start: 1961
active_end: "مستمر"
edges: []
related:
- id: "thk-michael-halliday", title: "مايكل هاليداي", type: "مفكر"
- id: "thk-jr-firth", title: "جون روبرت فيرث", type: "مفكر"
- id: "con-metafunctions-halliday", title: "الوظائفُ الفوقية الثلاث (Metafunctions)", type: "مفهوم"
- id: "wrk-introduction-functional-grammar-halliday", title: "مدخلٌ إلى النحو الوظيفي (An Introduction to Functional Grammar)", type: "عمل / كتاب"
- id: "thk-jr-martin", title: "ج. ر. مارتن", type: "مفكر"
- id: "con-genre-theory-sfl", title: "نظريةُ النوع النصّي في اللسانيات الوظيفية المنهجية (Genre Theory)", type: "مفهوم"
- id: "con-appraisal-theory-martin", title: "نظريةُ التقييم عند مارتن (Appraisal Theory)", type: "مفهوم"
- id: "con-transitivity-system-halliday", title: "نظامُ التعدّي وأنماطُ العمليات عند هاليداي (Transitivity System & Process Types)", type: "مفهوم"
- id: "con-mood-system-halliday", title: "نظامُ الصيغة عند هاليداي (Mood System)", type: "مفهوم"
- id: "con-theme-system-halliday", title: "نظامُ الموضوع عند هاليداي (Theme System)", type: "مفهوم"
- id: "con-cohesive-ties-halliday-hasan", title: "أدواتُ الاتساق الخمس عند هاليداي وحسن (Cohesive Ties)", type: "مفهوم"
- id: "thk-christian-matthiessen", title: "كريستيان ماتيسن", type: "مفكر"
gaps:
  - "عقدةٌ جذريةٌ (لا `belongs_to`) — تيارٌ وظيفيٌّ مستقلٌّ عن التوليدية والبنيوية الأمريكية، وإن استفاد من تراث مدرسة لندن (فيرث)."
  - "**سُدَّ 2026-09-21 (الدفعة 47)**: أُنشئ `thk-jr-firth` (أصلُ مدرسة لندن) و`con-metafunctions-halliday`."
---

# اللسانياتُ الوظيفيةُ المنهجية

إطارٌ نظريٌّ وظيفيٌّ أسّسه مايكل هاليداي منذ الستينيات، يصف بنيةَ اللغة بدلالة وظائفها التواصلية الاجتماعية بدلاً من صياغتها بقواعد تركيبيةٍ مجرَّدة عن السياق.

## الفوق-وظائف الثلاث

يفترض الإطارُ أن كلَّ ملفوظٍ لغويٍّ يحقّق في آنٍ واحد ثلاث وظائفَ متزامنة:

- **الوظيفةُ الفكرية (Ideational)**: تمثيلُ التجربة والمحتوى المعرفي (من فعل بمن ولماذا).
- **الوظيفةُ التفاعلية (Interpersonal)**: بناءُ العلاقة بين المتكلم والمخاطَب (سؤالٌ، أمرٌ، تعبيرٌ عن موقف).
- **الوظيفةُ النصية (Textual)**: تنظيمُ الخطاب في وحدةٍ متماسكةٍ ومترابطة (الموضوعُ والمعلَّق، الربطُ النصّي).

## المنهج

يحلّل هذا الإطارُ اللغةَ بوصفها **نسقاً من الخيارات (System Network)** — عند كل نقطةٍ في بناء الجملة يختار المتكلم من بين بدائلَ ممكنةٍ محدَّدة، والاختيارُ نفسُه يحمل دلالةً اجتماعية.

## الأثر

صار إطاراً معيارياً في علم اللغة التطبيقي، وتحليل الخطاب، وتعليم اللغة الثانية، ودراسات الأنواع النصية (Genre Studies)، خصوصاً خارج الولايات المتحدة حيث بقيت هيمنةُ النحو التوليدي أقلَّ رسوخاً.

## المصادر

- Halliday, M. A. K. (1985). *An Introduction to Functional Grammar*. Edward Arnold.
- Halliday, M. A. K.; Matthiessen, Christian M. I. M. (2014). *Halliday's Introduction to Functional Grammar* (4th ed.). Routledge.
