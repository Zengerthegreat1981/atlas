---
slug: "sch-communitarianism"
id: "SCH-0067"
type: "مدرسة"
part: "philosophy"
level: "متوسط"
title: "الجماعاتية"
en: "Communitarianism"
crumb: "الفلسفة ← السياسية المعاصرة ← الجماعاتية"
dates: "أمريكا، بريطانيا، كندا · من 1980s إلى اليوم"
country: "الولايات المتحدة، بريطانيا، كندا"
language: "الإنجليزية"
cultural_origin: "anglo-american"
active_start: 1981
active_end: "مستمر"
edges:
- rel: "evolved_from", target: "sch-aristotelianism", target_type: "مدرسة"
related:
- id: "sch-rawlsianism", title: "الليبرالية السياسية المعاصرة", type: "مدرسة"
- id: "sch-aristotelianism", title: "المشائية / الأرسطية", type: "مدرسة"
- id: "sch-civic-republicanism", title: "الجمهورية المدنية (Civic Republicanism)", type: "مدرسة"
- id: "con-communitarian-self", title: "الذات المتجذرة", type: "مفهوم"
- id: "con-recognition-struggle", title: "الاعتراف (النضال من أجله)", type: "مفهوم"
- id: "thk-macintyre", title: "ألاسدير ماكنتاير", type: "مفكر"
- id: "thk-sandel", title: "مايكل ساندل", type: "مفكر"
- id: "thk-walzer", title: "مايكل والزر", type: "مفكر"
- id: "con-communitarian-situated-self", title: "الذات المتجذرة والمجتمع التاريخي (The Situated Self)", type: "مفهوم"
gaps:
  - "**رُبطت عقدٌ معزولة 2026-09-08:** أُضيفت هنا إشارةٌ إلى عقدةٍ كانت لا يشير إليها أيُّ ملفّ، فكانت غيرَ قابلةٍ للوصول بالتنقّل. وكلُّ إشارةٍ عكسُ علاقةٍ يؤكّدها المستودعُ أصلاً (تأليفٌ أو انتماءٌ أو رابطٌ صادرٌ منها)، لم تُخترَع واحدةٌ منها."
  - "نصوص MacIntyre, Sandel, Taylor, Walzer, Etzioni تحتاج مراجعة من المصدر الأولي."
  - "المصادر الأولية تحتاج مراجعة وتوثيقاً إضافياً."
---

# الجماعاتية

تيار فلسفي-سياسي ينتقد الفردانية المجردة في الليبرالية السياسية، مُؤكّداً على أهمية المجتمع والتقاليد والقيم المشتركة في تحديد الذات.

## الأفكار المركزية

- **نقد الذات اللامُجتمعة**: الذات تتشكل في سياق مجتمع (ردّ على رولز).
- **القيم المشتركة**: لا يمكن لمجتمع ليبرالي أن يحيا بدون قيم مشتركة (Sandel).
- **الفضائل والروايات**: الفضيلة تحتاج سرداً تاريخياً ومؤسسات (MacIntyre).
- **تعدد المراكز (Pluralism)**: لا يمكن تقليص المعنى الأخلاقي لحسابات فردية (Walzer).
- **الترابط الاجتماعي (Social Capital)**: البناء الاجتماعي جزء من الرفاه (Putnam، Etzioni).

## الفلاسفة

- **أليسدير ماكنتاير (Alasdair MacIntyre)**: «After Virtue» (1981).
- **مايكل ساندل (Michael Sandel)**: «Liberalism and the Limits of Justice» (1982).
- **تشارلز تايلور (Charles Taylor)**: «Sources of the Self» (1989).
- **مايكل فالزر (Michael Walzer)**: «Spheres of Justice» (1983).
- **أميتاي إتزيوني (Amitai Etzioni)**: «The Spirit of Community» (1993).
- **روبرت بوتنام (Robert Putnam)**: «Bowling Alone» (1995).

## الأثر

- أثّرت في سياسة بيل كلينتون (الأمة الثالثة).
- تحوّلت لإطار في علم الاجتماع (Robert Putnam).
- أثّرت في سياسات الهوية والعرق في أمريكا الشمالية.

## القيد

- انتقاد بأنها تبرر القمع باسم المجتمع.
- صعوبة تعريف «المجتمع» بشكل ملموس.

## المصادر

- MacIntyre, Alasdair. *After Virtue: A Study in Moral Theory*. University of Notre Dame Press, 1981.
- Sandel, Michael. *Liberalism and the Limits of Justice*. Cambridge University Press, 1982.
- Taylor, Charles. *Sources of the Self: The Making of the Modern Identity*. Harvard University Press, 1989.
- Walzer, Michael. *Spheres of Justice: A Defense of Pluralism and Equality*. Basic Books, 1983.
- Etzioni, Amitai. *The Spirit of Community: Rights, Responsibilities, and the Communitarian Agenda*. Crown Publishers, 1993.
