---
slug: "sch-african-decolonial"
id: "SCH-0010"
type: "مدرسة"
part: "philosophy"
level: "متوسط"
title: "الفلسفة الأفريقية الديكولونيالية"
en: "African Decolonial Philosophy"
crumb: "الفلسفة ← الأفريقية ← الديكولونيالية"
dates: "أفريقيا، الـDiaspora · من 2000s إلى اليوم"
country: "متعددة"
cultural_origin: "african"
language: "متعددة"
active_start: 2000
active_end: "مستمر"
edges:
- rel: "belongs_to", target: "sch-decolonial-philosophy", target_type: "مدرسة"
related:
- id: "sch-pan-africanism", title: "عموم أفريقيا (Pan-Africanism)", type: "مدرسة"
- id: "sch-postcolonial-philosophy", title: "ما بعد الاستعمار الفلسفي", type: "مدرسة"
- id: "sch-afrocentrism", title: "أفريقية المركز (Afrocentrism)", type: "مدرسة"
- id: "thk-cabral", title: "أميليكار كابرال", type: "مفكر"
- id: "thk-mbembe", title: "أشيل مبيمبي", type: "مفكر"
- id: "con-occasionalism-ashari", title: "العناية الإلهية (العرضية عند الأشعري)", type: "مفهوم"
- id: "con-necropolitics-concept", title: "السياسة النكرو-سياسية وإدارة الموت (Necropolitics)", type: "مفهوم"
gaps:
  - "**فُرِّغ حقلُ الانتماء 2026-09-08:** كان `belongs_to` يشير إلى `sch-african-psychology` ختماً قالبياً — مدرسةٌ في علم النفس لا تصلح أباً لمدارسَ فلسفية؛ والمظلّةُ الصحيحة «الفلسفة الأفريقية» لا ملفَّ لها في الأطلس. وهذه مدرسةُ نِدٍّ أو مظلّةٌ مستقلّة، فبقي الحقلُ فارغاً؛ ولم يُوضَع أبٌ تقريبيٌّ لأنّ أباً خاطئاً أسوأُ من غياب أب."
  - "**رُبطت عقدٌ معزولة 2026-09-08:** أُضيفت هنا إشارةٌ إلى عقدةٍ كانت لا يشير إليها أيُّ ملفّ، فكانت غيرَ قابلةٍ للوصول بالتنقّل. وكلُّ إشارةٍ عكسُ علاقةٍ يؤكّدها المستودعُ أصلاً (تأليفٌ أو انتماءٌ أو رابطٌ صادرٌ منها)، لم تُخترَع واحدةٌ منها."
  - "نصوص Sabelo J. Ndlovu-Gatsheni, Lewis Gordon, Maldonado-Torres need review."
  - "المصادر الأولية تحتاج مراجعة وتوثيقاً إضافياً."
---
# الفلسفة الأفريقية الديكولونيالية
تيار فلسفي يُطبّق الديكولونيالية (كتيار أوسع من ما بعد الكولونيالية) على الفلسفة الأفريقية، مع التركيز على «إعادة بناء المعرفة» بعد الاستعمار.
## الأفكار المركزية
- **إعادة بناء المعرفة (Epistemic Decolonization)**: تفكيك المعرفة الأوروبية-المركزية.
- **استقلالية المعرفة الأفريقية**: بناء فلسفة متجذرة في الموروث الأفريقي.
- **نقد الحداثة الأوروبية**: لا فصل بين الحداثة والاستعمار.
- **خيال ما بعد-استعماري**: عند Mignolo وQuijano.
- **Māori-African Dialogues**: حوارات بين المعرفة المهمّشة.
## الفلاسفة
- **Sabelo J. Ndlovu-Gatsheni (سابيلو نتلوفو)**: «Coloniality of Power in Postcolonial Africa» (2013).
- **Lewis R. Gordon (لويس غوردون)**: الأفريقي-أمريكي.
- **Nelson Maldonado-Torres (نيلسون مالدونادو-توريس)**: من بورتوريكو.
- **Walter Mignolo (فالتر منيغولو)**: «The Darker Side of Western Modernity» (2011).
- **Aníbal Quijano (أنيبال كيخانو)**: «Coloniality of Power» (2000).
## الأثر
- أثّرت في حركات «African Renaissance» الأفريقية.
- أثّرت في الدراسات الأفريقية الأكاديمية.
- أثّرت في الإصلاح التعليمي في أفريقيا (Decolonizing the Curriculum).
## القيد
- تواجه انتقيدات من الحداثيين (تقليص المعرفة للعرق).
- تواجه صعوبة في التمييز عن الما بعد الكولونيالية.
## المصادر
- Quijano, A. (2000). "Coloniality of Power, Eurocentrism, and Latin America." *Nepantla: Views from South*, 1(3), 533–580.
- Mignolo, W. D. (2011). *The Darker Side of Western Modernity: Global Futures, Decolonial Options*. Duke University Press.
- Maldonado-Torres, N. (2007). "On the Coloniality of Being: Contributions to the Development of a Concept." *Cultural Studies*, 21(2–3), 240–270.
- Ndlovu-Gatsheni, S. J. (2013). *Coloniality of Power in Postcolonial Africa: Myths of Decolonization*. CODESRIA.
