---
slug: "sch-corpus-linguistics"
id: "SCH-9221"
type: "مدرسة"
part: "linguistics"
level: "متقدم"
linguistic_level: "lexicon"
cultural_origin: "anglo-european"
title: "لسانياتُ المدوّنات ومناهجُها (Corpus Linguistics)"
en: "Corpus Linguistics"
crumb: "علم اللغة ← اللغةُ والحاسوبُ والقياس ← لسانياتُ المدوّنات"
dates: "بريطانيا · من 1961 إلى الوقت الحاضر"
country: "بريطانيا، الولايات المتحدة"
language: "الإنجليزية"
active_start: 1961
active_end: "مستمر"
edges: []
related:
- id: "thk-john-sinclair", title: "جون سنكلير", type: "مفكر"
- id: "ins-grammaticality-judgments", title: "الاختباراتُ النحويةُ وحكمُ المتكلّم الأصليّ (Grammaticality Judgments)", type: "منهج/أداة بحث"
- id: "con-idioms-formulaic-language", title: "التعابيرُ الاصطلاحية واللغةُ الصيغية (Idioms & Formulaic Language)", type: "مفهوم"
- id: "con-semantic-prosody-corpus", title: "العروضُ الدلاليّ في لسانيات المدوّنات (Semantic Prosody)", type: "مفهوم"
- id: "thk-michael-hoey", title: "مايكل هوي", type: "مفكر"
- id: "con-lexical-priming-hoey", title: "التنشيطُ المعجميّ (Lexical Priming)", type: "مفهوم"
- id: "con-idiom-principle-open-choice", title: "مبدأُ الدليل النصّي ومبدأُ الاختيار المفتوح (Idiom Principle & Open-Choice Principle)", type: "مفهوم"
- id: "wrk-cobuild-dictionary-project", title: "قاموسُ Collins COBUILD ومشروعُ بنك اللغة الإنجليزية", type: "أثر"
- id: "con-brown-corpus-founding", title: "مدوّنةُ براون: أوّلُ مدوّنةٍ لغويةٍ إلكترونية ممثِّلة", type: "مفهوم"
- id: "thk-douglas-biber", title: "دوغلاس بايبر", type: "مفكر"
- id: "con-multidimensional-analysis-biber", title: "التحليلُ متعدِّدُ الأبعاد للتنوّع السجلّي عند بايبر (Multi-Dimensional Analysis)", type: "مفهوم"
- id: "con-keyword-keyness-analysis-corpus", title: "تحليلُ الكلمات المفتاحية والقيمةِ الإحصائية المميِّزة (Keyword & Keyness Analysis)", type: "مفهوم"
gaps:
  - "لا `belongs_to`/`evolved_from`: نشأت لسانياتُ المدوّنات كردِّ فعلٍ منهجيٍّ مباشر على الاعتماد التوليديّ على حدس المتكلم (`ins-grammaticality-judgments`)، لا كامتدادٍ تاريخيٍّ من مدرسةٍ سابقة؛ العلاقةُ نقديةٌ سُجِّلت بـ`related`."
---

# لسانياتُ المدوّنات ومناهجُها

مقاربةٌ منهجيةٌ في علم اللغة تعتمد على تحليل مجموعاتٍ ضخمةٍ من النصوص الفعلية المسجَّلة حاسوبياً (المدوّنات، Corpora) لاستخراج أنماط الاستعمال اللغوي الحقيقي، بدل الاعتماد على حدس المتكلم أو جملٍ مصطنَعة.

## من الحدس إلى الدليل الإحصائي

نشأت هذه المدرسةُ في الستينيات كردِّ فعلٍ على هيمنة منهج "أحكام المقبولية" التوليديّ (`ins-grammaticality-judgments`)، الذي ينتقده أنصارُ المدوّنات لاعتماده على حدسٍ فرديٍّ قد لا يعكس الاستعمالَ الفعليَّ للغة إحصائياً. تقيس لسانياتُ المدوّنات بدلاً من ذلك تكرارَ الأنماط والتراكيب فعلياً عبر ملايين الكلمات من نصوصٍ حقيقية.

## أدواتُ التحليل

تعتمد المدرسةُ على برمجياتٍ متخصّصة لاستخراج "السياقات المتراصّة" (Concordances، كلّ سياقات ظهور كلمةٍ معيَّنة) و"التلازمات اللفظية" (Collocations، الكلماتُ التي تظهر معاً بتكرارٍ أعلى من الصدفة الإحصائية)، بما يكشف أنماطَ استعمالٍ دقيقةً يصعب رصدُها بالحدس وحده.

## الأثر

أثّرت لسانياتُ المدوّنات بعمقٍ في المعجمية الحديثة (كمشروع COBUILD)، وتعليم اللغة الأجنبية، وصارت لاحقاً الأساسَ الإحصائيَّ لتدريب نماذج اللغة الحاسوبية الكبرى.

## المصادر

- Sinclair, John (1991). *Corpus, Concordance, Collocation*. Oxford University Press.
- Biber, Douglas; Conrad, Susan; Reppen, Randi (1998). *Corpus Linguistics: Investigating Language Structure and Use*. Cambridge University Press.
